...
void LayerNormGradCPUHelper(const T* dy, const T* x, const U* gamma,
  ...
  LayerNormGradCPU(dy_h, x_h, gamma_h, N, D, epsilon,
                   dgamma_h, dbeta_h, dx_h);
...
void LayerNormGPU(const T* x, const U* gamma, const U* beta, const U epsilon,
  ...
  if (use_single_warp) {
    ...
    LayerNormRowReduceInToOutWarp<<<
        DivUp(N, kBlockSize / kWarpSize), kBlockSize>>>(
...
void LayerNormGradGPU(const T* dy, const T* x, const U* cache_mean,
  ...
  LayerNormUpdate<<<DivUp(N * D, kBlockSize), kBlockSize>>>(
      dy, N, D, dx, dx_ops);
  cudaEventRecord(stop);
